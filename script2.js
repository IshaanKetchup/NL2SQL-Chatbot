...
const BACKEND_URL = 'https://nl2sql-chatbot.onrender.com'; // Default FastAPI port
...
function initializeEventListeners() {
    ...
    // Schema buttons
    document.getElementById('refresh-schema-btn').addEventListener('click', fetchSchemaFromBackend);
    document.getElementById('add-table-btn').addEventListener('click', addTable);
    document.getElementById('save-schema-btn').addEventListener('click', saveSchemaToBackend);
    document.getElementById('import-schema-btn').addEventListener('click', uploadSchema);
    document.getElementById('upload-schema-input-btn').addEventListener('click', uploadSchema);
...
function renderSchema() {
    ...
    schema.forEach((table, tableIndex) => {
        ...
        tableCard.dataset.tableIndex = tableIndex;
        tableCard.innerHTML = `
            ...
            <div class="space-y-1" id="columns-${tableIndex}">
                ${table.columns.map((col, colIndex) => `
                    <div class="column-row">
                        ...
                        <button class="remove-column-btn icon-btn w-6 h-6 hover:bg-red-900/20 hover:text-red-400" data-table-index="${tableIndex}" data-column-index="${colIndex}">
            ...
            <div class="mt-3 pt-3 border-t border-neutral-800 flex gap-2">
                <button class="add-column-btn btn btn-secondary flex items-center gap-1 text-sm flex-1" data-table-index="${tableIndex}">
                    <i data-feather="plus" class="w-3 h-3"></i>
                    Add Column
                </button>
                <button class="remove-table-btn px-3 py-1.5 text-sm font-medium rounded-md text-red-400 hover:bg-red-900/20 border border-red-900/30 transition-colors" data-table-index="${tableIndex}">
                    Remove
                </button>
            </div>
        `;
    ...
    // Attach event listeners to dynamically created elements using event delegation
    attachDynamicEventListeners();
}

function attachDynamicEventListeners() {
    // Event delegation for table header clicks (edit table name)
    schemaBody.addEventListener('click', function(e) {
        const tableHeader = e.target.closest('.table-header');
        if (tableHeader) {
            const tableCard = tableHeader.closest('.table-card');
            const tableIndex = parseInt(tableCard.dataset.tableIndex);
            editTableName(tableIndex, tableHeader);
        }
        
        // Handle column name clicks
        const columnNameText = e.target.closest('.column-name-text');
        if (columnNameText) {
            const columnRow = columnNameText.closest('.column-row');
            const tableCard = columnRow.closest('.table-card');
            const tableIndex = parseInt(tableCard.dataset.tableIndex);
            const columnIndex = Array.from(columnRow.parentElement.children).indexOf(columnRow);
            editColumnName(tableIndex, columnIndex, columnRow);
        }
        
        // Handle remove column button clicks
        const removeColumnBtn = e.target.closest('.remove-column-btn');
        if (removeColumnBtn) {
            const tableIndex = parseInt(removeColumnBtn.dataset.tableIndex);
            const columnIndex = parseInt(removeColumnBtn.dataset.columnIndex);
            removeColumn(tableIndex, columnIndex);
        }
        
        // Handle add column button clicks
        const addColumnBtn = e.target.closest('.add-column-btn');
        if (addColumnBtn) {
            const tableIndex = parseInt(addColumnBtn.dataset.tableIndex);
            addColumn(tableIndex);
        }
        
        // Handle remove table button clicks
        const removeTableBtn = e.target.closest('.remove-table-btn');
        if (removeTableBtn) {
            const tableIndex = parseInt(removeTableBtn.dataset.tableIndex);
...
function addTable() {
    ...
    setTimeout(() => {
        ...
        const lastTableCard = document.querySelector(`.table-card[data-table-index="${lastTableIndex}"]`);
        if (lastTableCard) {
            const lastTableHeader = lastTableCard.querySelector('.table-header');
            if (lastTableHeader) {
...
function addColumn(tableIndex) {
    ...
    setTimeout(() => {
        ...
        const tableCard = document.querySelector(`.table-card[data-table-index="${tableIndex}"]`);
        if (tableCard) {
            const lastColumnRow = tableCard.querySelector(`#columns-${tableIndex} .column-row:last-child`);
            if (lastColumnRow) {
